...
router.post("/set-free-mode", async (req, res) => {
  ...
  try {
    await pool.query(
      "UPDATE users SET is_free = true WHERE id = $1",
      [userId]
    );
...
router.get("/check-free-mode/:id", async (req, res) => {
  const { id } = req.params;

  try {
    const result = await pool.query(
      "SELECT is_free FROM users WHERE id = $1",
      [id]
    );
